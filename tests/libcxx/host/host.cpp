...
void* host_enclave_thread(void* args)
{
...

    {
        ...
        _enclave_host_id_map[thread_args->enc_key] = thread_id;
    }
    ...
    // Launch the thread
    oe_result_t result =
        enc_enclave_thread(thread_args->enclave, thread_args->enc_key);
    OE_TEST(result == OE_OK);
...
void host_create_thread(uint64_t enc_key, oe_enclave_t* enclave)
{
    ...
    printf("***** host_create_thread() enc_key=%lu: check 0\n", enc_key);
    ...
    printf(
        "\nhost_create_thread() enc_key=%lu - mapped_thread_id=%#10lx\n",
        enc_key,
        mapped_thread_id);
